	...
	) 
{
	...
	// --- Control-system: treat relay/all/debug REST-like routes as CGI even without ".cgi"
    // Allows URLs like: /<APIKEY>/relay1/on  and  /<APIKEY>/all/state  and  /<APIKEY>/debug
    if (strstr(buf, "/relay") || strstr(buf, "/all") || strstr(buf, "/debug"))
	{
		*type = PTYPE_CGI;
	}
	else if 	(strstr(buf, ".htm")	|| strstr(buf, ".html"))	*type = PTYPE_HTML;
	...
	// --- Control-system: treat relay/all REST-like routes as CGI even without ".cgi"
    // Allows URLs like: /<APIKEY>/relay1/on  and  /<APIKEY>/all/state
    else if (strstr(buf, "/relay") || strstr(buf, "/all"))
        *type = PTYPE_CGI;
	else 													*type = PTYPE_ERR;
}
